...
{
    public sealed partial class AssignShipmentOrderGrid : UserControl
    {
        
        static HttpClient client = new HttpClient();
        public AssignShipmentOrderGrid()
        {
            InitializeComponent();
            Windows.Storage.ApplicationDataContainer localSettings = Windows.Storage.ApplicationData.Current.LocalSettings;
            int CarrierID = (int) localSettings.Values["CarrierID"];
            InitAssignedshipmentOrdergrid(CarrierID).GetAwaiter();
           

        }
        public async Task InitAssignedshipmentOrdergrid(int id)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                shipmentOrderdataGrid.ItemsSource = ShipmentOrders.Where(x=>x.CarrierId == id && x.DriverId != null);
